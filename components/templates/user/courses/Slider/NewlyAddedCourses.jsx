...
export default function NewlyAddedCourses() {
  ...
  useEffect(() => {
    fetchByFilter("newlyaddedcourses");
  }, []);

  return (
    <div className="">
      <div className="mb-5">
        <SectionTitle text={"جدیدترین ها"} />
      </div>
      <div className="rounded-sm bg-gray-50 p-3">
        <Swiper
          ...
          modules={[Navigation]}
          navigation={{
            nextEl: ".swiper-button-newly-next-custom-1",
            prevEl: ".swiper-button-newly-prev-custom-1",
        ...
        >
          ...
          <button className="swiper-button-newly-next-custom-1 text-primaryGray1 absolute top-[-50%] bottom-[-50%] -left-4 z-20 cursor-pointer px-5">
            <ChevronLeft className="h-7 w-7 rounded-full bg-cyan-500 p-1 text-white hover:bg-cyan-500/85 md:h-9 md:w-9" />
          </button>
          <button className="swiper-button-newly-prev-custom-1 text-primaryGray1 absolute top-[-50%] -right-4 bottom-[-50%] z-20 cursor-pointer px-5">
